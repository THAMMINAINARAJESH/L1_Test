...
QUESTION_BANK_DIR = r"C:\Users\RajeshThamminaina\PycharmProjects\gmail_outlook\Read_AT_Commands\001_fixed\l1\mixed_both_files (1)\mixed_both_files\question_bank"
...
OUTPUT_DIRECTORY = r"C:\Users\RajeshThamminaina\PycharmProjects\gmail_outlook\Read_AT_Commands\001_fixed\mixed_both_files\videos"
...
def upload_recordings():
    ...
    # Save webcam
    if webcam:
        webcam_path = os.path.join(folder, f"{timestamp}_webcam{suffix}.webm")
        webcam.save(webcam_path)
        print(f"✅ Saved webcam to: {webcam_path}")
    else:
        print(f"⚠️ No webcam file received for {candidate_id}")

    # Save screen
    if screen:
        screen_path = os.path.join(folder, f"{timestamp}_screen{suffix}.webm")
        screen.save(screen_path)
        print(f"✅ Saved screen to: {screen_path}")
    else:
        print(f"⚠️ No screen file received for {candidate_id}")
        ...
        if existing_session:
            if existing_session.get ( 'is_completed' ):
                ...
                if existing_candidate_token:
                    # MODIFIED: Added _external=True
                    return redirect ( url_for ( 'test_page', candidate_user_id=candidate_user_id_for_redirect,
                                                candidate_token=existing_candidate_token, _external=True ) )
                else:
                    return render_template ( 'link_expired.html',
        ...
        if candidate_session:
            if candidate_session['is_completed']:
                ...
                return redirect ( url_for ( 'submitted_page',
                                            ...
                                            email=candidate_session.get ( 'email', 'N/A' ),
                                            answered_count=len ( candidate_session.get ( 'answers', {} ) ), _external=True ) )
            else:
                # MODIFIED: Added _external=True
                return redirect ( url_for ( 'test_page', candidate_user_id=existing_candidate_id,
                                            candidate_token=test_sessions[existing_candidate_id]['token'], _external=True ) )
        else:
...
def register_and_start_test(link_hash):
        ...
        if candidate_session:
            if candidate_session.get ( 'is_completed' ):
                return jsonify ( {"message": "You have already completed this test. This link is no longer active.",
                                  ...
                                  "test_url": url_for ( 'submitted_page',
                                                        ...
                                                        email=candidate_session.get ( 'email', 'N/A' ),
                                                        answered_count=len (
                                                            candidate_session.get ( 'answers', {} ) ), _external=True ),
                                  "completed": True} ), 200
            ...
            return jsonify ( {
                ...
                "test_url": url_for ( 'test_page', candidate_user_id=existing_candidate_id,
                                      candidate_token=candidate_session['token'], _external=True )
            } ), 200
        else:
    ...
    return jsonify ( {
        ...
        "test_url": url_for ( 'test_page', candidate_user_id=candidate_user_id,
                              candidate_token=candidate_token, _external=True )} ), 200
...
def force_submit(candidate_user_id):
    ...
    # MODIFIED: Added _external=True
    return redirect ( url_for ( 'submitted_page', name=name, email=email, answered_count=answered_count, _external=True ) )
